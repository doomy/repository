...
final class TableDefinitionFactoryTest extends TestCase
{
    public function testCreateTableDefinition(): void
    {
        ...
        $this->assertEquals('varcharColumn', $columns[0]->getName());
        $this->assertEquals(ColumnType::VARCHAR, $columns[0]->getColumnType());
        $this->assertEquals(255, $columns[0]->getLength());
        $this->assertFalse($columns[0]->isPrimaryKey());
        $this->assertFalse($columns[0]->isIdentity());
        $this->assertTrue($columns[0]->isUnique());

        $this->assertEquals('intColumn', $columns[1]->getName());
        $this->assertEquals(ColumnType::INTEGER, $columns[1]->getColumnType());
        $this->assertEquals(null, $columns[1]->getLength());
        $this->assertTrue($columns[1]->isPrimaryKey());
        $this->assertTrue($columns[1]->isIdentity());
        $this->assertFalse($columns[1]->isUnique());
    }
}
